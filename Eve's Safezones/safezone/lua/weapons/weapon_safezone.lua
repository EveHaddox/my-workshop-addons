...
function SWEP:Deploy()
    hook.Add("PostDrawOpaqueRenderables","safezones_display",function()
...
function SWEP:Holster()
    ...
    self.pos_one = nil
    self.pos_two = nil
    hook.Remove("PostDrawOpaqueRenderables","safezones_display")
    return true
end
...
function SWEP:DrawHUD()
    draw.SimpleText("Leftclick = place first point", "Trebuchet24", 10, ScrH()/2.5, evefrtxt, TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP)
    draw.SimpleText("Rightclick = place second point & save", "Trebuchet24", 10, ScrH()/2.5+40, evefrtxt, TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP)
    draw.SimpleText("Reload = Open menu to delete safezones", "Trebuchet24", 10, ScrH()/2.5+80, evefrtxt, TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP)
end
...
function SWEP:Reload()
    ...
    function self.window:Paint(w,h)
        draw.RoundedBox(6, 0, 0, w, h, Color(32, 34, 37))
        draw.RoundedBox(6, 1, 1, w - 2, h - 2, Color(54, 57, 62))
    end
    ...
    for k,v in pairs(text) do
        ...
        helpText:SetText(v)
        helpText:SetTextColor(Color(218,218,218))
    ...
    local safezones = {}
    for k,v in ipairs(ents.FindByClass("safezone")) do
    ...
    for k,v in pairs(safezones) do
        ...
        zonebutton.DoClick = function(s)
            net.Start("safezone_delete")
            net.WriteString(s.id)
            net.SendToServer()
            self.window:Close()
        end
        function zonebutton:Paint(w,h)
            draw.RoundedBox(6, 0, 0, w, h, Color(47, 49, 54))
            if self.Hovered then
                draw.RoundedBox(6, 0, 0, w, h, Color(66, 70, 77))
            end
        end
